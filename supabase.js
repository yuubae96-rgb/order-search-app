...
function quoteLatestPrice(materialId){
  return QUOTE_MATERIAL_PRICES.filter(p => String(p.material_id) === String(materialId)).sort((a,b) => {
...
function parsePurchaseDimensions(material){
  if(material.width_mm && material.length_value){
    const lengthMm = material.length_unit === 'm' ? qNum(material.length_value) * 1000 : qNum(material.length_value);
...
function calculateQuoteMaterialSnapshot(){
  ...
  QUOTE_MATERIAL_SNAPSHOT = { materialId: material.id, priceId: price ? price.id : null, unitPrice: price ? qNum(price.price) : null, estimatedCost: estimatedCost == null ? null : Math.round(estimatedCost) };
  if(info){
    const priceText = price ? `最新単価：<b>${qNum(price.price).toLocaleString('ja-JP')}円</b> / ${price.price_basis === 'kg' ? 'kg' : material.stock_unit}　適用日 ${price.effective_from}` : '<b>単価未登録</b>';
    const costText = estimatedCost == null ? 'サイズ・数量・購入寸法などが揃うと材料原価の参考値を表示します。' : `材料原価（ロス未加算）：<b>${Math.round(estimatedCost).toLocaleString('ja-JP')}円</b>　${detail}`;
    info.innerHTML = `${priceText}<br>${costText}`;
  }
}
function applyMasterMaterial(){
  const select = document.getElementById('f_materialMaster');
  if(!select || !select.value){ calculateQuoteMaterialSnapshot(); return; }
  const material = QUOTE_MATERIALS.find(m => String(m.id) === String(select.value));
  if(!material) return;
  ...
  if(matSelect){ matSelect.value = legacy; matSelect.dispatchEvent(new Event('change', {bubbles:true})); }
...
async function loadQuoteMaterials(){
  ...
  if(!session){ select.innerHTML = '<option value="">ログイン後に材料を読み込みます</option>'; return; }
  ...
  if(mRes.error || pRes.error){ console.error(mRes.error || pRes.error); select.innerHTML = '<option value="">材料マスターの読込に失敗しました</option>'; return; }
  QUOTE_MATERIALS = mRes.data || [];
  QUOTE_MATERIAL_PRICES = pRes.data || [];
  select.innerHTML = '<option value="">材料マスターを使わない</option>' + QUOTE_MATERIALS.map(m => `<option value="${m.id}">${materialLabel(m)}</option>`).join('');
  if(!QUOTE_MATERIALS.length){ const info=document.getElementById('materialMasterInfo'); if(info) info.innerHTML = '材料マスターはまだ0件です。材料・在庫で登録すると、ここから選べるようになります。'; }
}
async function saveQuoteMaterialSnapshot(estimateId){
  ...
  const { error } = await supabaseClient.from('estimates').update({ material_id:s.materialId, material_price_id:s.priceId, material_unit_price_snapshot:s.unitPrice, material_cost_snapshot:s.estimatedCost }).eq('id', Number(estimateId));
...
function installQuoteMaterialIntegration(){
  ...
  box.className = 'field'; box.style.marginBottom = '12px';
  box.innerHTML = `<label>材料マスターから選択<span class="opt">推奨</span></label><select id="f_materialMaster"><option value="">読み込み中...</option></select><div id="materialMasterInfo" style="font-size:12px;line-height:1.65;margin-top:7px;opacity:.78;">材料マスターを読み込んでいます。</div>`;
  row.parentNode.insertBefore(box, row);
  document.getElementById('f_materialMaster').addEventListener('change', applyMasterMaterial);
  ['f_sizeV','f_sizeH','f_quantity'].forEach(id => document.getElementById(id)?.addEventListener('input', calculateQuoteMaterialSnapshot));
  if(typeof StorageAPI !== 'undefined' && !StorageAPI.__materialLinked){
    const originalAdd = StorageAPI.add.bind(StorageAPI);
    StorageAPI.add = async function(order){ const saved = await originalAdd(order); await saveQuoteMaterialSnapshot(saved && saved.id); return saved; };
    const originalUpdate = StorageAPI.update.bind(StorageAPI);
    StorageAPI.update = async function(id, patch, expectedUpdatedAt){ const result = await originalUpdate(id, patch, expectedUpdatedAt); if(result && !result.conflict) await saveQuoteMaterialSnapshot(id); return result; };
    StorageAPI.__materialLinked = true;
  }
  loadQuoteMaterials();
}
document.addEventListener('DOMContentLoaded', installQuoteMaterialIntegration);
supabaseClient.auth.onAuthStateChange((_event, session) => { if(session) setTimeout(loadQuoteMaterials, 0); });

// 材料・在庫管理を見積アプリ本体に統合する追加モジュール
const materialsInlineScript = document.createElement('script');
materialsInlineScript.src = 'materials-inline.js?v=20260818-2342';
materialsInlineScript.async = false;
document.head.appendChild(materialsInlineScript);
