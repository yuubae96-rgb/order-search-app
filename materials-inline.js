...
(function(){
  ...
  function opt(a){return '<option value="">選択してください</option>'+a.map(v=>`<option value="${esc(v)}">${v==='㎡'?'㎡（平方メートル）':esc(v)}</option>`).join('')}
  ...
  function displayUnit(m,p){if(p?.price_basis==='kg')return 'kg';if(m?.category==='シール・ラベル')return '㎡';return m?.stock_unit||''}
  ...
  function build(){
    ...
    screen.innerHTML=`
      ...
      <div id="mi-master" class="mi-pane"><div class="card"><div class="card-title">材料マスター登録</div><div class="mi-grid">
        ...
        <div class="field"><label>購入サイズ・形態</label><select id="miPurchase"></select></div><div class="field"><label>在庫単位</label><select id="miUnit"><option value="">選択してください</option><option value="枚">枚</option><option value="巻">巻</option><option value="缶">缶</option><option value="kg">kg</option><option value="個">個</option><option value="㎡">㎡（平方メートル）</option></select></div>
        <div class="field" id="miAdhField" style="display:none"><label>粘着</label><select id="miAdh"></select></div><div class="field" id="miLamField" style="display:none"><label>ラミネート</label><select id="miLam"></select></div>
    ...
    $('miCat').innerHTML=opt(Object.keys(CATALOG));$('miAdh').innerHTML=opt(ADH);$('miLam').innerHTML=opt(LAM);$('miUnit').innerHTML=opt(UNITS);
    ...
    $('miList').addEventListener('click',e=>{const b=e.target.closest('.mi-delete-btn');if(b)deleteMaterial(Number(b.dataset.id))});
  ...
  function refreshConditionalFields(){const show=$('miCat')?.value==='シール・ラベル';if($('miAdhField'))$('miAdhField').style.display=show?'':'none';if($('miLamField'))$('miLamField').style.display=show?'':'none';if(!show){if($('miAdh'))$('miAdh').value='';if($('miLam'))$('miLam').value=''}}
  function refreshMat(){const c=CATALOG[$('miCat').value]||{};$('miMat').innerHTML=opt(Object.keys(c));refreshDef();refreshConditionalFields()}
  function refreshDef(){const d=(CATALOG[$('miCat').value]||{})[$('miMat').value];$('miSpec').innerHTML=opt(d?.specs||[]);$('miThick').innerHTML=opt(d?.thickness||[]);$('miPurchase').innerHTML=opt(d?.purchase||[]);$('miUnit').innerHTML=opt(UNITS);if(d&&UNITS.includes(d.unit))$('miUnit').value=d.unit}
  ...
  function render(){if(!$('miList'))return;const q=$('miSearch').value.trim().toLowerCase();const list=mats.filter(m=>[m.name,m.category,m.spec,m.supplier].join(' ').toLowerCase().includes(q));$('miList').innerHTML=list.length?list.map(m=>{const s=stock(m.id),p=latestPrice(m.id),warn=Number(m.reorder_point)>0&&s<=Number(m.reorder_point),u=displayUnit(m,p);return `<div class="mi-item"><div><b>${esc(m.name)}</b><div class="mi-meta">${[m.category,m.spec,m.thickness_mm?('t'+m.thickness_mm):'',m.purchase_form,m.supplier].filter(Boolean).map(esc).join(' / ')}${p?'<br>最新単価 '+fmt(p.price)+'円 / '+esc(u)+' ('+esc(p.effective_from)+')':''}</div><button type="button" class="mi-delete-btn" data-id="${m.id}">削除</button></div><div class="mi-stock ${warn?'mi-alert':''}">${fmt(s)} ${esc(u)}</div></div>`}).join(''):'<div class="empty-state">まだ材料がありません</div>';$('miCount').textContent=mats.length;$('miAlert').textContent=mats.filter(m=>Number(m.reorder_point)>0&&stock(m.id)<=Number(m.reorder_point)).length;$('miValue').textContent=fmt(Math.round(mats.reduce((z,m)=>z+stock(m.id)*unitValue(m,latestPrice(m.id)),0)))+'円';$('miMvMat').innerHTML=mats.map(m=>{const p=latestPrice(m.id),u=displayUnit(m,p);return `<option value="${m.id}">${esc(m.name)}（${fmt(stock(m.id))} ${esc(u)}）</option>`}).join('');$('miHistory').innerHTML=moves.slice(0,150).map(x=>{const m=mats.find(y=>String(y.id)===String(x.material_id)),u=displayUnit(m,latestPrice(m?.id));return `<div class="mi-item"><div>${esc(x.movement_date)}　${x.movement_type==='in'?'入庫':x.movement_type==='out'?'出庫':'棚卸'}<div class="mi-meta">${esc(m?.name||'材料')} ${esc(x.destination||'')}</div></div><b>${fmt(x.quantity)} ${esc(u)}</b></div>`}).join('')}
  async function deleteMaterial(id){const m=mats.find(x=>Number(x.id)===Number(id));if(!m)return;if(!confirm('「'+m.name+'」を削除しますか？\n\nテスト登録など、不要な材料だけ削除してください。'))return;const related=moves.filter(x=>Number(x.material_id)===Number(id));if(related.length){alert('この材料には入出庫履歴があるため削除できません。\n本番データを誤って消さないための保護です。');return}const {error:pe}=await supabaseClient.from('material_prices').delete().eq('material_id',id);if(pe){console.error(pe);alert('単価履歴の削除に失敗しました');return}const {error}=await supabaseClient.from('materials').update({active:false}).eq('id',id);if(error){console.error(error);alert('材料の削除に失敗しました');return}alert('「'+m.name+'」を削除しました');await load()}
  async function saveMaterial(){const cat=$('miCat').value,name=$('miMat').value,spec=$('miSpec').value,th=$('miThick').value,pur=$('miPurchase').value,unit=$('miUnit').value;if(!cat||!name||!unit){alert('大分類・材料・在庫単位を選んでください');return}const isLabel=cat==='シール・ラベル';const row={name,category:cat,spec:spec||null,thickness_mm:(!th||th==='-')?null:Number(th),purchase_form:pur||null,stock_unit:unit,unit_weight_kg:$('miWeight').value||null,supplier:$('miSupplier').value.trim()||null,reorder_point:Number($('miReorder').value)||0,storage_location:$('miLocation').value.trim()||null,notes:$('miNotes').value.trim()||null,adhesive_type:isLabel?($('miAdh').value||null):null,laminate_type:isLabel?($('miLam').value||null):null};const {data,error}=await supabaseClient.from('materials').insert(row).select().single();if(error){console.error(error);alert('材料登録に失敗しました');return}const price=Number($('miPrice').value)||0;if(price>0)await supabaseClient.from('material_prices').insert({material_id:data.id,effective_from:$('miPriceDate').value||today(),price,price_basis:$('miPriceBasis').value,supplier:row.supplier});const initial=Number($('miInitial').value)||0;if(initial!==0){const {data:{user}}=await supabaseClient.auth.getUser();await supabaseClient.from('inventory_movements').insert({material_id:data.id,movement_date:today(),movement_type:'adjust',quantity:initial,destination:'初期登録',memo:'初期在庫',created_by:user?.email||null})}alert('材料を登録しました');await load()}
