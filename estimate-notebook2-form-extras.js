...
(function(){
  ...
  const owner=()=>{try{const p=parent.getCurrentOperator&&parent.getCurrentOperator();if(p&&String(p.name||'').trim()==='承認(社長)')return true;const label=String(pd.getElementById('whoBtn')?.textContent||'').trim();return label.startsWith('承認(社長)')}catch(e){return false}};
  ...
  async function loadOwnerLogs(){const logs=pd.getElementById('logs');if(!logs)return;if(!owner()){logs.innerHTML='<div class="empty">操作履歴は社長のみ閲覧できます</div>';return}try{const j=await historyApi({action:'list_logs',limit:500}),a=j.items||[];logs.innerHTML=a.length?a.map(x=>{const e=x.estimate_notebook2||{},target=[e.company_name,e.product_name].filter(Boolean).join(' / ');return '<div class="log"><div class="row"><div class="grow"><div class="time">'+jp(x.created_at)+'　'+esc(x.operator_name)+'</div><div class="action">'+esc(x.action)+'</div>'+(target?'<div class="target">'+esc(target)+'</div>':'')+(x.details?'<div class="role">'+esc(x.details)+'</div>':'')+'</div><button class="btn red historyDeleteOne" data-log-id="'+x.id+'" type="button">削除</button></div></div>'}).join(''):'<div class="empty">まだ履歴はありません。</div>';logs.querySelectorAll('.historyDeleteOne').forEach(b=>b.onclick=async()=>{if(!owner())return alert('操作履歴の削除は社長のみ実行できます。');if(!confirm('この操作履歴を1件削除しますか？'))return;try{await historyApi({action:'delete_log',id:Number(b.dataset.logId)});await loadOwnerLogs()}catch(e){alert(e.message)}})}catch(e){logs.textContent=e.message}}
  function installBulk(){const history=pd.getElementById('history');if(!history)return;const row=history.querySelector('.card .row');if(!row)return;let b=pd.getElementById('deleteAllLogs');if(!b){b=pd.createElement('button');b.id='deleteAllLogs';b.type='button';b.className='btn red';b.textContent='履歴を一括削除';b.onclick=async()=>{if(!owner())return alert('操作履歴の削除は「承認(社長)」のみ実行できます。');if(!confirm('操作履歴をすべて削除しますか？\nこの操作は元に戻せません。'))return;try{await historyApi({action:'delete_all_logs'});await loadOwnerLogs();alert('操作履歴をすべて削除しました')}catch(e){alert(e.message)}};row.appendChild(b)}b.style.display=owner()?'inline-block':'none';const reload=pd.getElementById('reloadLogs');if(reload)reload.onclick=loadOwnerLogs}
  try{parent.loadLogs=loadOwnerLogs}catch(e){}
  installBulk();syncPermission();setInterval(()=>{installBulk();syncPermission();const pane=pd.getElementById('history');if(pane&&pane.classList.contains('active')&&owner()&&pd.getElementById('logs')?.textContent.includes('社長のみ閲覧できます'))loadOwnerLogs()},500);
})();