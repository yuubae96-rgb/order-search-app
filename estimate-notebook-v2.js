...
(function(){
  ...
  function mount(){
    ...
    form.addEventListener('submit',async()=>{
      ...
      const context={
        editingId:(window.state&&state.editingId)?String(state.editingId):null,
        company:$('f_company').value.trim(),
        orderDate:$('f_orderDate').value,
        itemName:$('f_itemName').value.trim(),
        orderNumber:$('f_orderNumber').value.trim()
      };
      setTimeout(()=>saveForEstimate(context),850);
    },true);

    const titleObs=new MutationObserver(()=>{
      if(window.state && state.editingId && String(state.editingId)!==lastLoadedEditId){
        lastLoadedEditId=String(state.editingId); loadForEstimate(state.editingId);
      }
      if(!window.state || !state.editingId) lastLoadedEditId=null;
    });
    titleObs.observe($('formCardTitle'),{childList:true,subtree:true});

    ['recentList','searchResults'].forEach(id=>{
      const el=$(id); if(!el) return;
      new MutationObserver(()=>scheduleDecorate(el)).observe(el,{childList:true,subtree:true});
      scheduleDecorate(el);
    });
  }
  ...
  function refreshWon(){
    ...
    const old=sel.value;
    sel.innerHTML='<option value="">選択してください</option>'+options.map((o,i)=>o.quantity?`<option value="${i}">${o.quantity}枚 / @${fmt(o.unitPrice||0)}円</option>`:'').join('');
    sel.value=old;
    markWonOption();
    sel.onchange=markWonOption;
  }
  function markWonOption(){
    document.querySelectorAll('.quote-option').forEach(x=>x.classList.remove('won-option'));
    if($('v2_result').value!=='won') return;
    const i=$('v2_wonOption').value;
    if(i!=='') document.querySelector(`.quote-option[data-i="${i}"]`)?.classList.add('won-option');
  }
  function syncResult(){
    const v=$('v2_result').value;
    $('wonFields').style.display=v==='won'?'':'none';
    $('lostFields').style.display=v==='lost'?'':'none';
    markWonOption();
  }
  async function findSavedEstimateId(context){
    if(context.editingId) return context.editingId;
    let q=supabaseClient.from('estimates').select('id').eq('company_name',context.company).eq('estimate_date',context.orderDate);
    if(context.orderNumber) q=q.eq('estimate_number',context.orderNumber);
    if(context.itemName) q=q.eq('product_name',context.itemName);
    const {data}=await q.order('created_at',{ascending:false}).limit(1).maybeSingle();
    return data&&data.id?String(data.id):null;
  }
  async function saveForEstimate(context){
    if(!window.supabaseClient) return;
    const id=await findSavedEstimateId(context); if(!id)return;
    const valid=options.filter(x=>Number(x.quantity)>0);
    const selected=$('v2_result').value==='won' ? $('v2_wonOption').value : '';
    const wonSourceIndex=selected===''?-1:Number(selected);
    const selectedObject=wonSourceIndex>=0?options[wonSourceIndex]:null;
    await supabaseClient.from('estimate_quantity_options').delete().eq('estimate_id',Number(id));
    if(valid.length){
      await supabaseClient.from('estimate_quantity_options').insert(valid.map((o,i)=>({
        estimate_id:Number(id),quantity:Number(o.quantity),unit_price:Number(o.unitPrice)||0,
        amount:Number(o.quantity)*(Number(o.unitPrice)||0),sort_order:i,
        is_won:!!(selectedObject && Number(o.quantity)===Number(selectedObject.quantity) && Number(o.unitPrice||0)===Number(selectedObject.unitPrice||0))
      })));
    }
    const won=selectedObject;
    await supabaseClient.from('estimates').update({
      won_quantity:won?Number(won.quantity):null,won_unit_price:won?Number(won.unitPrice):null,
      result_date:$('v2_resultDate').value||null,
      lost_reason:$('v2_result').value==='lost'?($('v2_lostReason').value||null):null,
      result_note:$('v2_resultNote').value||null
    }).eq('id',Number(id));
  ...
  async function loadForEstimate(id){
    ...
    const [{data:rows},{data:e}]=await Promise.all([
      supabaseClient.from('estimate_quantity_options').select('*').eq('estimate_id',Number(id)).order('sort_order'),
      supabaseClient.from('estimates').select('status,won_quantity,won_unit_price,result_date,lost_reason,result_note').eq('id',Number(id)).maybeSingle()
    ]);
    options=(rows&&rows.length)?rows.map(r=>({quantity:r.quantity,unitPrice:r.unit_price,isWon:r.is_won})):[{quantity:$('f_quantity').value||'',unitPrice:$('f_quantity').value?Math.round(Number($('f_amount').value||0)/Number($('f_quantity').value)):''}];
    renderOptions();
    const map={'受注':'won','失注':'lost','回答待ち':'pending'};
    $('v2_result').value=map[e&&e.status]||'pending';
    $('v2_resultDate').value=e&&e.result_date||'';
    $('v2_lostReason').value=e&&e.lost_reason||'';
    $('v2_resultNote').value=e&&e.result_note||'';
    if(rows){const wi=rows.findIndex(r=>r.is_won); if(wi>=0)$('v2_wonOption').value=String(wi);}
    syncResult();
  }
  ...
  function scheduleDecorate(container){
    ...
    decorateTimer=setTimeout(()=>decorateCards(container),120);
  }
  async function decorateCards(container){
    if(!window.supabaseClient||!container)return;
    const cards=[...container.querySelectorAll('.order-card')].filter(c=>!c.dataset.notebookV2);
    if(!cards.length)return;
    const ids=cards.map(c=>Number(c.dataset.id)).filter(Boolean); if(!ids.length)return;
    const [{data:rows},{data:estimates}]=await Promise.all([
      supabaseClient.from('estimate_quantity_options').select('*').in('estimate_id',ids).order('sort_order'),
      supabaseClient.from('estimates').select('id,status,won_quantity,won_unit_price,result_date,lost_reason,result_note').in('id',ids)
    ]);
    const byId={}; (rows||[]).forEach(r=>(byId[r.estimate_id]||(byId[r.estimate_id]=[])).push(r));
    const eById={}; (estimates||[]).forEach(e=>eById[e.id]=e);
    cards.forEach(card=>{
      const id=Number(card.dataset.id), qs=byId[id]||[], e=eById[id]||{};
      const actions=card.querySelector('.actions');
      const wrap=document.createElement('div'); wrap.className='notebook-v2';
      let html='<div class="notebook-v2-head"><span>数量別 見積単価</span><span>紙ノートの結果記録</span></div>';
      if(qs.length){
        html+=qs.map(q=>`<div class="notebook-v2-row ${q.is_won?'won':''}"><span class="q">${fmt(q.quantity)}枚${q.is_won?'　受注':''}</span><span class="p">@${fmt(q.unit_price)}円</span><span class="a">${fmt(q.amount)}円</span></div>`).join('');
      }else{
        html+='<div class="notebook-v2-row"><span class="q">数量別価格 未登録</span><span></span><span></span></div>';
      }
      const st=e.status==='受注'?'won':e.status==='失注'?'lost':'pending';
      if(st==='won'){
        html+=`<div class="notebook-v2-result won"><strong>受注</strong>${e.won_quantity?`　${fmt(e.won_quantity)}枚 / @${fmt(e.won_unit_price)}円`:''}${e.result_date?`<span class="notebook-v2-date">${fmtDate(e.result_date)}</span>`:''}${e.result_note?`<br>${esc(e.result_note)}`:''}</div>`;
      }else if(st==='lost'){
        html+=`<div class="notebook-v2-result lost"><strong>失注</strong>${e.lost_reason?`　<span class="notebook-v2-reason">${esc(e.lost_reason)}</span>`:''}${e.result_date?`<span class="notebook-v2-date">${fmtDate(e.result_date)}</span>`:''}${e.result_note?`<br>${esc(e.result_note)}`:''}</div>`;
      }else{
        html+='<div class="notebook-v2-result pending"><strong>回答待ち</strong></div>';
      }
      wrap.innerHTML=html;
      if(actions) card.insertBefore(wrap,actions); else card.appendChild(wrap);
      card.dataset.notebookV2='1';
    });
  }
